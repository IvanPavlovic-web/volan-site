...
function extendMaterial(BaseMaterial: typeof THREE.MeshStandardMaterial, cfg: ExtendedMaterialConfig) {
  ...
  const uniforms = THREE.UniformsUtils.clone(
    baseUniforms as THREE.ShaderMaterial["uniforms"],
  ...
  for (const [includeTag, code] of Object.entries(cfg.vertex ?? {})) {
    vert = vert.replace(includeTag, `${includeTag}\n${code}`);
  }

  for (const [includeTag, code] of Object.entries(cfg.fragment ?? {})) {
    frag = frag.replace(includeTag, `${includeTag}\n${code}`);
  }
...
const PlaneNoise = ({ material, width, count, height }: PlaneNoiseProps) => {
  ...
  useFrame((_, delta) => {
    ...
    (mat.uniforms.time as { value: number }).value += 0.1 * delta;
  });
...
const DirLight = ({ position, color }: DirLightProps) => {
  ...
  return <directionalLight ref={dirRef} color={color} intensity={1} position={position} />;
};
